...
face_data = np.asarray(face_data)  # Normalize pixel values
face_data = face_data.reshape(100, -1)

# Save the face data and labels
if 'names.pkl' not in os.listdir('data/'):
    # Ensure names is a list with 100 labels
    names = [name] * 100
    ...
    # Ensure the number of names matches the number of faces
    if len(names) == len(face_data):
        names += [name] * 100
    else:
        print(f"Error: Inconsistent number of samples! Names: {len(names)}, Faces: {len(face_data)}")
        exit()
